...
/**
 * Event manager for subscribing to and publishing Armada job events.
 * Uses a pub/sub pattern where subscribers can register for events on specific job set IDs.
 *
...
public class ArmadaEventManager<T> {
  ...
  /**
   * Get the number of subscribers for a specific job set ID.
   *
   * @param jobSetId the job set ID
   * @return the number of subscribers
   */
  public int getSubscriberCount(String jobSetId) {
    CopyOnWriteArrayList<Consumer<T>> consumerList = subscribers.get(jobSetId);
    return consumerList != null ? consumerList.size() : 0;
  }
}
